...
plot_test_power <- function(dist, n, params, inter, MCS=100) {
  
  gensample <- if (dist == "norm") {
    function() {
      return(rnorm(n, params[1], params[2]))
    }
  } else {
    function() {
      return(rexp(n, params[1]))
    }
  }
  ...
  for (i in seq_along(inter)) {
    ...
    for (j in seq_len(MCS)) {
      sample <- gensample()
      z_correct = z_correct - ztest(sample, mu)$wynik
      t_correct = t_correct - ttest(sample, mu)$wynik
      w_correct = w_correct - wtest(sample, mu)$wynik
    }
